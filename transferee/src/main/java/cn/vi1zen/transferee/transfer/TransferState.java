...
import cn.vi1zen.transferee.utils.ImageProcessor;
import cn.vi1zen.transferee.utils.ImageUtils;
import cn.vi1zen.transferee.view.image.TransferImage;

import static android.view.ViewGroup.LayoutParams.MATCH_PARENT;
import static android.widget.ImageView.ScaleType.FIT_CENTER;
import static cn.vi1zen.transferee.utils.ImageUtils.TYPE_GIF;
...
public abstract class TransferState {
    ...
    void startPreview(final TransferImage targetImage, final File source,
        ...
        final File cacheDir = transfer.getTransConfig().getCacheDir();
                ...
                cacheDir, getDisplaySize(), new ImageProcessor.ImageProcessCallback() {
                    ...
                    public void onSuccess(File file) {
                        if (ImageUtils.getImageType(file) == TYPE_GIF) {
                            try {
                                Glide.with(targetImage).load(file).into(targetImage);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        } else {
    ...
    /**
     * 从网络或者从指定的缓存中加载 SourceImageList.get(position) 对应的图片
     *
